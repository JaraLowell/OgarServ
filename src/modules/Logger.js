...
function info(message) {
    writeCon(colorWhite, LogLevelEnum.INFO, message);
    writeLog(LogLevelEnum.INFO, message);
};

function warn(message) {
    writeCon(colorYellow, LogLevelEnum.WARN, message);
...
function writeDebug(message) {
    writeCon(colorBlue + colorBright, LogLevelEnum.INFO, message);
    writeLog(LogLevelEnum.DEBUG, message);
};
...
function writeCon(color, level, message) {
    ...
    var prefix = "[" + getTimeString() + "] ";
    process.stdout.write(prefix + message + "\u001B[0m" + EOL);
};
