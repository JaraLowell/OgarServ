...
BotPlayer.prototype.decide = function(cell) {
    ...
    var algorithm = function(check, influence) {
        ...
        if (check.cellType == 0 && cell.mass / 2.2 > check.mass && cell.mass / 5 < check.mass &&
            !split && splitCooldown == 0 && cellLength < 6) {
        ...
        } else if (check.cellType == 0 && check.mass > cell.mass / 2.6 && splitCooldown == 0) {
            if (distance < cell.getSize() * 3) {
                splitTarget = check;
                splitAway = true;
                split = true;
            }
    ...
    if (split) {
        if (threats.length > 0) {
            if (this.largest(threats).mass / 2.6 > cell.mass && !splitAway) {
                ...
                var deltaY = splitTarget.position.y - cellPos.y;
                var deltaX = splitTarget.position.x - cellPos.x;
                ...
                // Direction to move
                this.mouse = {
                    x: cellPos.x + (750 * Math.cos(angle)),
                    y: cellPos.y + (750 * Math.sin(angle))
                };
                this.splitCooldown = 7;
                this.gameServer.splitCells(this);
                return;
            }
        }
